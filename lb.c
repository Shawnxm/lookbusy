...
typedef void (*spinner_fn)(long long, long long, long long, int, void*, void *);
...
static uint64_t get_cpu_busy_time(int cpuid)
{
    FILE *f;
    char s[2560];
    ...
    char *pattern =
        "^cpu+([0-9]+)[[:space:]]+([0-9]+)[[:space:]]+([0-9]+)[[:space:]]+([0-9]+)"; 
    //say(1, "pattern  %s\n",pattern);
    //pattern[4]='0'+0;
    //say(1, "pattern after %s\n",pattern);
    ...
    say(1, "cpu_spin(%d): current utime=%"PRIu64
...
static void cpu_spin_calibrate(int util, uint64_t *busycount, suseconds_t *sleeptime)
{
    ...
    *sleeptime = 100000*(1- (float)util/100.0);
    //*sleeptime = elapsed * util / 100;
    
    say(1, "cpu_spin (%d): %"PRIu64" iterations in %lld usec (%lld/sec)\n",
...
static void cpu_spin(long long ncpus, long long util_l, long long util_h, int cpuid, void *dummy, void *dummy2)
{
    uint64_t busycount;
    ...
    cpu_spin_calibrate(util, &busycount, &sleeptime);
		...
		sleeptime = (suseconds_t)10*1000;//to get the load more sensitive

    say(1, "cpu_spin (%d): spinning cpu\n", getpid());
    while (1) {
        struct timeval tv;
        long long counter;
        ...
        if (! first) {
            ...
            if ((adjust < 0 && oldadjust > 0) ||
                ...
                say(1, "cpu_spin (%d): adjusted approximately correctly\n",
                       getpid());
                say(1, "cpu_spin (%d): spin count %"PRIu64"\n", getpid(), busycount);
            }
        } else {
            first = 0;
        }
        ...
        busytime = get_cpu_busy_time(cpuid);

        say(1, "cpu_spin (%d): spinning (0 to %"PRIu64")...\n", getpid(), busycount);
        for (counter = 0; counter < busycount; counter++) {
            squander_time(counter);
        }

        say(1, "cpu_spin (%d): sleeping...\n", getpid());
        usleep(sleeptime);
        gettimeofday(&tv, NULL);
        walltime2 = tv.tv_sec * 1000000 + tv.tv_usec;
        busytime2 = get_cpu_busy_time(cpuid);
        ...
        say(1, "cpu_spin (%d): %"PRIu64" iterations; %"PRIu64" CPU-jiffies elapsed\n",
...
static void mem_stir(long long asz, long long dummy, long long dummy2, int cpuid, void *dummyp, void *dummyp2)
{
...
static void disk_churn(long long dummy0, long long dummy, long long dummy2, int cpuid, void *pathv, void *szv)
{
...
static pid_t fork_and_call(char *desc, spinner_fn fn, long long arg1, long long arg2, long long arg3, int cpuid, void *argP, void *argP2)
{
    pid_t p = fork();
    if (p == -1) {
        perror("fork");
        ...
        (*fn)(arg1, arg2, arg3, cpuid, argP, argP2);
        exit(0);
    } else {
...
static pid_t *start_cpu_spinners(int *ncpus, int* util)
{
    pid_t *pids;
    if (*ncpus <= 0)
        *ncpus = get_cpu_count();
    ...
    int i;
    say(1, "cpu_spin (%d): starting %d spinner(s)\n",
           getpid(), *ncpus);
    for (i = 0; i < *ncpus; i++) {
        pids[i] = fork_and_call("CPU spinner", cpu_spin, *ncpus, util[i], util[i], i, NULL, NULL);
		int mask = pow (2, i);
		int err, syscallres;
        syscallres = syscall(__NR_sched_setaffinity, pids[i], sizeof(mask), &mask);
		    if (syscallres)
		    {
		        err = errno;
		        say(1, "Error in the syscall setaffinity: mask=%d=0x%x err=%d=0x%x", mask, mask, err, err);
		    }
    }
    return pids;
}
...
static pid_t *start_disk_stirrer(off_t util, char **paths, size_t paths_n)
{
    ...
    for (i = 0; i < paths_n; i++) {
        ...
        p[i] = fork_and_call(desc, disk_churn, 0, 0, 0,0 , paths[i], &util);
    }
    return p;
}
...
{
    return fork_and_call("mem stirrer", mem_stir, sz, 0, 0, 0, NULL, NULL);
}
...
"CPU usage options:\n"
"  -z, --cpu-util=PCT,  Desired utilization of each CPU, in percent.\n"
"  -c, --cpu-util=PCT,  Desired utilization of each CPU, in percent.\n"
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv,
        ...
        switch (c) {
            ...
            case 'b':
                ...
                break;
            case 'z':
                c_cpu_util = (int *) malloc(ncpus+1);
                printf("ncpus: %d\n",ncpus);
                int i=0;
                char * token = strtok(optarg, ",");
                while( token != NULL ) {
                    c_cpu_util[i]=atol(token);
                    token = strtok(NULL, ",");
                    i++;
                    if(i>=ncpus) break;
                }
    ...
    if (ncpus != 0) {
        cpu_pids = start_cpu_spinners(&ncpus, c_cpu_util); // forks
        n_cpu_pids = ncpus;
    }
    if (c_disk_util != 0) {
